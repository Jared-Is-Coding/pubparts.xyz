...
export default ({ resourceList }: ResourceListSearchbarProps) => {
	...
	const resourceTypeCheckboxes = Object.fromEntries(uniqueResourceTypes.map((p) => [p, false])) as {
		[P in ResourceType]: boolean
	}
	...
	useEffect(() => {
		...
		setShowCopySearchButton(!!(searchText || Object.values(checkedTypeBoxes).some((v) => !!v)))
				...
				<Stack direction="vertical" gap={3}>
					...
					{uniqueResourceTypes.length > 1 && (
						<div className="searchTypeCheckBoxes">
							<Form.Label as="h3">Resource Type(s):</Form.Label>
							...
							link={
								!windowIsDefined()
									...
									: "http://" +
										...
										`&type=${uniqueResourceTypes.filter((t) => !!checkedTypeBoxes[t])}`
							}
							style={{ display: showCopySearchButton ? "initial" : "none", maxWidth: "max-content" }}
						/>
					</Stack>
