...
export default ({ partList }: ItemListSearchbarProps) => {
	...
	const partTypeCheckboxes = Object.fromEntries(uniquePartTypes.map((p) => [p, false])) as {
		[P in PartType]: boolean
	}
	const fabricationMethodCheckboxes = Object.fromEntries(uniqueFabricationMethods.map((p) => [p, false])) as {
		[P in FabricationMethod]: boolean
	}
	...
	if (!didMount.current && windowIsDefined()) {
		...
		// Set checkboxes as checked
		const fabricationMethod = (queryParams.get("fab")?.split(",") ??
			queryParams.get("fabrication")?.split(",") ??
			[]) as FabricationMethod[]
	...
	useEffect(() => {
		...
		items.forEach((item) => {
			...
			if (
				...
				(searchText && !dataPartTitle?.toLowerCase().includes(searchText.toLowerCase())) ||
				...
				<Stack direction="vertical" gap={3}>
					...
					{uniquePartTypes.length > 1 && (
						<div className="searchTypeCheckBoxes">
							<Form.Label as="h3">Part Type(s):</Form.Label>
					...
					{uniqueFabricationMethods.length > 1 && (
						<div className="searchFabricationCheckBoxes">
							<Form.Label as="h3">Fabrication Method(s):</Form.Label>

							<ButtonGroup size="sm">
								{uniqueFabricationMethods
									.sort((a, b) => a.localeCompare(b))
									.map((f, index) => (
										<ToggleButton
							...
							link={
								!windowIsDefined()
									...
									: "http://" +
										...
										`&type=${uniquePartTypes.filter((t) => !!checkedTypeBoxes[t])}` +
										`&fab=${uniqueFabricationMethods.filter((f) => !!checkedFabricationMethodBoxes[f])}`
							}
							style={{ display: showCopySearchButton ? "initial" : "none", maxWidth: "max-content" }}
						/>
					</Stack>
