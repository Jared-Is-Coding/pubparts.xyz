...
async function runOne(reason: string): Promise<void> {
	...
	console.log(
		`[parts-sync] completed reason=${result.reason} catalog=${result.catalogCount} shop=${result.shopCount} resources=${result.resourcesCount} durationMs=${result.durationMs}`,
	)
}
