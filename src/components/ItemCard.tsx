...
export default (item: ItemData, index: number) => {
	...
	const lightboxImageUrls = getImageUrls(item.imageSrc)
		...
		<Col
			...
			<Card>
				...
				<div
					...
					{item.imageSrc && <span role="img" aria-label={"Preview imagine of part, " + item.title}></span>}
				</div>

				{/* Part image lightbox */}
				{/* https://fslightbox.com/react */}
				{item.imageSrc && <Lightbox src={lightboxImageUrls} toggler={lightboxToggler} />}

				{/* Part type badges */}
				{item.typeOfPart?.length && (
					<Stack className="display-over-top-right" direction="vertical" gap={1}>
						{item.typeOfPart.map((p, pillIndex) => (
							<Badge key={`item-card-${index}-pill-t-${pillIndex}`} pill bg="dark">
								{toTitleCase(p)}
							</Badge>
						))}

						{item.fabricationMethod.map((f, pillIndex) => (
							<Badge key={`item-card-${index}-pill-f-${pillIndex}`} pill bg="dark">
								{f}
							</Badge>
						))}
					</Stack>
				...
				<Card.Body>
					...
					{(item.externalUrl || item.dropboxUrl) && (
						<Stack direction="vertical" gap={1}>
							...
							{item.dropboxUrl && (
								<Card.Link href={item.dropboxUrl} target="_blank">
									ZIP Download
									{!!item.dropboxZipLastUpdated && <>{` (${item.dropboxZipLastUpdated})`}</>}
								</Card.Link>
							)}
						</Stack>
					)}
