...
export async function bootstrapPartsDatabase(): Promise<PartsBootstrapResult> {
		...
		await prisma.$transaction(
			async (tx) => {
				await tx.$executeRawUnsafe(
					"TRUNCATE TABLE parts_catalog, shop_items, resources, platforms, part_types, resource_types, fabrication_methods, sync_state RESTART IDENTITY CASCADE",
				)
	...
	await writeLocalSnapshots(
		allParts as ItemData[],
		allPartsShopItems as PartsShopData[],
		allResources as ResourceData[],
	)
