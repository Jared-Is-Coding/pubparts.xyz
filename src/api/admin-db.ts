...
async function triggerProductionBuild(reason: string): Promise<BuildTriggerResult> {
	...
	try {
		...
		return {
			...
			ok: false,
			message:
				error instanceof Error ? `Build hook request failed: ${error.message}` : "Build hook request failed.",
		}
	}
}
...
async function hydratePartRelations(
	tx: Prisma.TransactionClient,
	partId: number,
	part: AdminPartPayload,
): Promise<void> {
...
async function hydrateShopRelations(
	tx: Prisma.TransactionClient,
	shopItemId: number,
	item: AdminShopPayload,
): Promise<void> {
...
async function fetchAllData(): Promise<{
	...
	const [partsRows, resourceRows, shopRows, pendingPartRows, pendingResourceRows, partTypeRows, resourceTypeRows] =
		await Promise.all([
			...
			(prisma as any).partPending
				.findMany({
					orderBy: { createdAt: "desc" },
				})
				.catch(() => []),
			(prisma as any).resourcePending
				.findMany({
					orderBy: { createdAt: "desc" },
				})
				.catch(() => []),
	...
	const parts = partsRows.map((part) => {
		...
		return {
			...
			title: part.title,
			imageSrc,
			fabricationMethod: part.fabricationMethods.map(
				(entry) => entry.fabricationMethod.name as FabricationMethod,
			),
