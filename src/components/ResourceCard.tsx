		...
		<Col
			...
			<Card className={`resource-${resource.typeOfResource}`}>
				<Card.Body>
					...
					{resource.typeOfResource.length > 0 && (
						<Stack className="display-over-top-right" direction="vertical" gap={1}>
							{resource.typeOfResource.map((r, pillIndex) => (
								<Badge key={`resource-card-${index}-pill-${pillIndex}`} pill bg="dark">
									{toTitleCase(r)}
								</Badge>
							))}
						</Stack>
						...
						<Card.Title as="h3">
										...
										link={
											!windowIsDefined()
												...
												: "http://" +
													...
													`?search=${encodeURIComponent(resource.title)}`
										}
									/>
									<> | </>
							...
							{!resource.externalUrl && (resource.appStoreLink || resource.playStoreLink) && (
								<>
									...
									{resource.title}
									<> on the </>
									{resource.appStoreLink && (
										<>
											<a href={resource.appStoreLink} target="_blank">
												App Store
											</a>
										</>
									)}
									{resource.playStoreLink && (
										<>
											{" "}
											or{" "}
											<a href={resource.playStoreLink} target="_blank">
												Play Store
											</a>
										</>
									)}
					...
					{resource.description && <Card.Text>{resource.description}</Card.Text>}
				</Card.Body>
			</Card>
		</Col>
	)
